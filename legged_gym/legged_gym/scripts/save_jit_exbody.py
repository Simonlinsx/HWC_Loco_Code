...
class HardwareRefNN(nn.Module):
    def __init__(self,  num_prop,
        ...
        # self.estimator = Estimator(num_obs=num_prop, num_history = 10,  num_latent = 16, activation = 'elu' , decoder_hidden_dims=[256, 128, 64], encoder_hidden_dims=[512, 256]).to('cpu')
        
        self.estimator = Estimator(num_obs=num_prop, output_dim=num_prop + 43 + 19 * 2 + 5, num_history = 10,  num_latent = 16, activation = 'elu' , decoder_hidden_dims=[256, 128, 64], encoder_hidden_dims=[512, 256]).to('cpu')
    ...
    def forward(self, obs):
        ...
        backbone_input = torch.cat([obs[:, : self.text_feat_input_dim + self.num_prop + self.num_demo],  latent], dim=1)
